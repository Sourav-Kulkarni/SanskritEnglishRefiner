...
function removeNewlinesFromSelection(textarea) {
  ...
  // 1. Ensure the textarea is focused (required for execCommand)
  textarea.focus();
  // 2. Use execCommand to overwrite selection while preserving Undo history
  const success = document.execCommand("insertText", false, cleaned);
  // 3. Fallback: If execCommand fails (rare), use your original method
  if (!success) {
    textarea.setRangeText(cleaned, start, end, "end");
  }
  // 4. Trigger your existing UI updates
  syncField(textarea);
  autoResize(textarea);
}
...
function loadFile(file) {
  ...
  reader.onload = (e) => {
    try {
      ...
      switchToEditor();
      showToast(
        `Loaded ${data.length} verse${data.length !== 1 ? "s" : ""} from "${file.name}"`,
        "success",
      );
    } catch (err) {
...
function buildInserter(before) {
  ...
  el.querySelector(".inserter-btn").addEventListener("click", () =>
    insertAt(before),
  );
  return el;
}
...
function buildCard(item, idx) {
  ...
  card
    ...
    .addEventListener("input", (e) => syncField(e.target));
...
editToggle.addEventListener("change", () => {
  ...
  const areas = Array.from(container.querySelectorAll("textarea.edit-area"));

  if (areas.length > 400) {
    showToast("Large file detected. Preparing edit mode…", "success");
  }
  ...
  function processBatch() {
    ...
    for (let j = 0; j < batchSize && i < areas.length; j++, i++) {
      autoResize(areas[i]);
    }
...
document.addEventListener("keydown", (e) => {
  ...
  if (mod && (key === "j" || key === "J")) {
    ...
    if (active && active.tagName === "TEXTAREA") {
      e.preventDefault();
      removeNewlinesFromSelection(active);
    }
...
function saveJSON() {
  ...
  let maxSeen = nonempty.reduce((max, item) => {
    ...
    return isFinite(n) && String(item.map_id).trim() !== ""
      ? Math.max(max, n)
      : max;
  }, 0);
  ...
  if (pruned > 0)
    msg += ` · ${pruned} blank verse${pruned !== 1 ? "s" : ""} removed`;
